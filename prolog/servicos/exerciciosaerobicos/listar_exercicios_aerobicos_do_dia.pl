:- module(listar_exercicios_aerobicos_do_dia, [listar_exercicios_aerobicos_do_dia/2]).

listar_exercicios_aerobicos_do_dia(Data, NomeUsuario):-
    absolute_file_name("users/", CaminhoAbsoluto),
    atomic_list_concat([CaminhoAbsoluto, NomeUsuario, "/", Data, "/aerobicos.txt"], PastaUsuario),
    leitura_exercicios_aerobicos(Data, PastaUsuario, Leitura),
    remover_ultimo(Leitura, Lista),
    listar_exercicios(Lista).


leitura_exercicios_aerobicos(_, PastaUsuario, Leitura):-
